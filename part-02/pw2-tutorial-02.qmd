...
Luigi's Eisdiele in München hat saisonal stark schwankende Verkaufszahlen (in hundert Litern Eis). Es gibt vier Quartale pro Jahr. Luigi hat die Verkaufszahlen der letzten **zwei** Jahre gesammelt:
...
3.  Prognostizieren Sie die bereinigten Werte ($\widehat{a}_{d,t} + \widehat{b}_{d,t} \cdot i$) für die vier Quartale des nächsten Jahres (Jahr 3, Q1 bis Q4). Hier ist $t$ der Index des letzten Beobachtungspunktes der bereinigten Reihe.
...
plt.plot(df_task4['Periode'], df_task4['Verkauf_desaisoniert'], marker='s', linestyle='--', label='Désaisonierter Verkauf $Y_d = Y_t/S_t$')

# Trend der désaisonierten Daten (a_d_hat_t)
# plt.plot(df_task4['Periode'], df_task4['a_d_hat_t'], marker='x', linestyle=':', label='Geglättetes Niveau (Trend) der dés. Daten $\widehat{a}_{d,t}$')

# Resaisonierte Prognosen für Jahr 3
plt.plot(df_prognosen_desaisoniert_j3['Periode'], df_prognosen_desaisoniert_j3['Prognose_Final'], marker='*', linestyle='-', color='red', markersize=10, label='Resaisonierte Prognose $P_{t+i} = (\widehat{a}_{d,t} + \widehat{b}_{d,t} \cdot i) \cdot S_{t+i}$')

plt.title(f'Eisverkauf: Ist, Désaisoniert und Prognose (Holt-Winters Mult., $\\alpha$={alpha_t4})')
...
# Plot für Aufgabe 2
plt.figure(figsize=(7, 5))
plt.plot(df_task2['Monat'], df_task2['Verkauf'], marker='o', linestyle='-', label='Tatsächlicher Verkauf $y_t$')
# plt.plot(df_results_t2['t'], df_results_t2['p_t'], marker='x', linestyle='--', color='gray', label='Ein-Schritt-Prognose $p_t = \widehat{a}_{t-1}+\widehat{b}_{t-1}$')
plt.plot(df_results_t2['t'], df_results_t2['a_hat_t'], marker='s', linestyle=':', label='Geschätztes Niveau $\widehat{a}_t$')
plt.plot(df_prognosen_t2_future['Monat'], df_prognosen_t2_future['Prognose'], marker='*', linestyle='-', color='red', markersize=10, label='Zukunftsprognose $p_{8+i}$')
...
Das Verfahren aus Übung 1 (Exponentielle Glättung mit Trendkorrektur)verwendet einen einzigen Glättungsfaktor ($\alpha$).
Das Niveau und der Trend werden durch eine doppelte exponentielle Glättung derselben Zeitreihe abgeleitet. Aus diesen beiden Reihen werden dann die Schätzungen für Niveau ($\widehat{a}t$) und Trend ($\widehat{b}_t$) berechnet.
...
Das Niveau ($\widehat{a}t$) wird direkt aus dem aktuellen Beobachtungswert ($y_t$) und der vorherigen Schätzung für Niveau und Trend ($\widehat{a}{t-1}+\widehat{b}{t-1}$) berechnet.
Der Trend ($\widehat{b}t$) wird direkt aus der Veränderung des Niveaus ($\widehat{a}_t - \widehat{a}{t-1}$) und der vorherigen Trendschätzung ($\widehat{b}{t-1}$) berechnet.
