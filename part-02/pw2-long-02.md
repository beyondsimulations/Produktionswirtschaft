...
(C) Univ.-Prof. Dr. Michael Manitz# Bestimmung der Saisonfaktoren 
...
![img-18.jpeg](img-18.jpeg)# Beispiel Saisonale Nachfrageschwankungen 
...
Verwendung von drei GlättungsparameternVerwendung von drei Glättungsparametern
Achsenabschnitt der Trendgeraden
...
$(t=1,2, \ldots, n)$Schätzwerte für die Regressionskoeffizienten
$\widehat{\beta_{j}}$ bzw. $b_{j}$
$j=0,1,2, \ldots, m)$
geschätzte ex-post-Prognosewerte
$\widehat{y}_{t}$ bzw. $p_{t}$
...
$\mathbf{b}_{(2 \times 1)}=\left(\mathbf{X}^{\mathbf{T}} \mathbf{X}\right)^{-1} \mathbf{X}_{(2 \times 2)}^{\mathbf{T}} \cdot \mathbf{y}_{(2 \times 1)}$Lineares Gleichungssystem (in Matrixschreibweise):
$\left[\begin{array}{cc}\sum_{k=t-n+1}^{t} & y_{k} \cdot k \\ \sum_{k=t-n+1}^{t} & y_{k} \cdot k\end{array}\right]=\left[\begin{array}{cc}n & \sum_{k=t-n+1}^{t} k \\ \sum_{k=t-n+1}^{t} & k \sum_{k=t-n+1}^{t} k^{2}\end{array}\right] \cdot\left[\begin{array}{l}b_{0} \\ b_{1}\end{array}\right]$
Es sei dann:
$\mathbf{y}:=\left[\begin{array}{l}y_{1} \\ y_{2}\end{array}\right]_{(2 \times 1-\text { Matrix })} \quad \mathbf{X}:=\left[\begin{array}{ll}1 & x_{11} \\ 1 & x_{12}\end{array}\right]_{(2 \times 2-\text { Matrix })} \quad \mathbf{b}:=\left[\begin{array}{l}b_{0} \\ b_{1}\end{array}\right]_{(2 \times 1-\text { Matrix })}$
Koeffizientenschätzung (Normalgleichungssystem in Matrixschreibweise):
...
Mengeneinheiten
![img-29.jpeg](img-29.jpeg)# Beispiel Exponentielle Glättung mit Trendkorrektur $(\alpha=0.1)$ 

Mengeneinheiten
![img-30.jpeg](img-30.jpeg)# Beispiel Einfach-Regression bei Vorliegen eines linearen Trends 

Mengeneinheiten
![img-31.jpeg](img-31.jpeg)# Üblicherweise: Einfache lineare Regressionsrechnung 

## Beispiel Einfach-Regression bei Vorliegen eines linearen Trends
...
## Trendgerade: Einfache lineare Regressionsrechnung

Beispiel 2 Einfach-Regression bei Vorliegen eines linearen Trends

| Monat $t$ | Jan | Feb | Mar | Apr | Mai | Jun | Jul | Aug | Sep | Okt | Nov | Dez |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| Beobachtungen | 30 | 25 | 37 | 43 | 39 | 55 | 66 | 59 | 48 | 53 | 49 | 57 |# Üblicherweise: Einfache lineare Regressionsrechnung 
...
$$
R^{2}=54.67366 \%
$$# Erweiterung der linearen Regressionsrechnung um weitere EinflussgrößenLineares Gleichungssystem:
(1) $\sum_{k=t-n+1}^{t} y_{k}=n \cdot b_{0}+b_{1} \cdot \sum_{k=t-n+1}^{t} k$
(2) $\sum_{k=t-n+1}^{t} y_{k} \cdot k=b_{0} \cdot \sum_{k=t-n+1}^{t} k+b_{1} \cdot \sum_{k=t-n+1}^{t} k^{2}$Lineares Gleichungssystem:
(1) $\sum_{k=t-n+1}^{t} y_{k} \cdot 1=b_{0} \cdot n+b_{1} \cdot \sum_{k=t-n+1}^{t} k$
(2) $\sum_{k=t-n+1}^{t} y_{k} \cdot k=b_{0} \cdot \sum_{k=t-n+1}^{t} k+b_{1} \cdot \sum_{k=t-n+1}^{t} k^{2}$Lineares Gleichungssystem:

$$
\begin{aligned}
& \sum_{t=1}^{n} y_{t} \cdot 1=b_{0} \cdot n \quad+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \\
& \sum_{t=1}^{n} y_{t} \cdot x_{1 t}=b_{0} \cdot \sum_{t=1}^{n} x_{1 t}+b_{1} \cdot \sum_{t=1}^{n} x_{1 t}^{2}
\end{aligned}
$$Lineares Gleichungssystem:

$$
\begin{aligned}
& \sum_{t=1}^{n} y_{t} \cdot 1=b_{0} \cdot n \quad+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot 1 \\
& \sum_{t=1}^{n} y_{t} \cdot x_{1 t}=b_{0} \cdot \sum_{t=1}^{n} x_{1 t}+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot x_{1 t}
\end{aligned}
...
$$
\begin{aligned}
& \sum_{t=1}^{n} y_{t} \cdot 1=b_{0} \cdot n \quad+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot 1+b_{2} \cdot \sum_{t=1}^{n} x_{2 t} \cdot 1+\cdots+b_{m} \cdot \sum_{t=1}^{n} x_{m t} \cdot 1 \\
& \sum_{t=1}^{n} y_{t} \cdot x_{1 t}=b_{0} \cdot \sum_{t=1}^{n} x_{1 t}+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot x_{1 t}+b_{2} \cdot \sum_{t=1}^{n} x_{2 t} \cdot x_{1 t}+\cdots+b_{m} \cdot \sum_{t=1}^{n} x_{m t} \cdot x_{1 t}
\end{aligned}
$$# System der Normalgleichungen 
...
& \sum_{t=1}^{n} y_{t} \cdot 1=b_{0} \cdot n \quad+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot 1+b_{2} \cdot \sum_{t=1}^{n} x_{2 t} \cdot 1+\cdots+b_{m} \cdot \sum_{t=1}^{n} x_{m t} \cdot 1 \\
& \sum_{t=1}^{n} y_{t} \cdot x_{1 t}=b_{0} \cdot \sum_{t=1}^{n} x_{1 t}+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot x_{1 t}+b_{2} \cdot \sum_{t=1}^{n} x_{2 t} \cdot x_{1 t}+\cdots+b_{m} \cdot \sum_{t=1}^{n} x_{m t} \cdot x_{1 t} \\
& \sum_{t=1}^{n} y_{t} \cdot x_{2 t}=b_{0} \cdot \sum_{t=1}^{n} x_{2 t}+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot x_{2 t}+b_{2} \cdot \sum_{t=1}^{n} x_{2 t} \cdot x_{2 t}+\cdots+b_{m} \cdot \sum_{t=1}^{n} x_{m t} \cdot x_{2 t}
\end{aligned}
$$# System der Normalgleichungen 
...
$\sum_{t=1}^{n} y_{t} \cdot x_{m t}=b_{0} \cdot \sum_{t=1}^{n} x_{m t}+b_{1} \cdot \sum_{t=1}^{n} x_{1 t} \cdot x_{m t}+b_{2} \cdot \sum_{t=1}^{n} x_{2 t} \cdot x_{m t}+\cdots+b_{m} \cdot \sum_{t=1}^{n} x_{m t} \cdot x_{m t}$# System der Normalgleichungen 

Es ist dann (in Matrixschreibweise):
$\mathbf{y}:=\left[\begin{array}{c}y_{1} \\ y_{2} \\ \vdots \\ y_{n}\end{array}\right](n \times 1-$ Matrix $)$
$\mathbf{X}:=\left[\begin{array}{ccccc}1 & x_{11} & x_{21} & \cdots & x_{m 1} \\ 1 & x_{12} & x_{22} & \cdots & x_{m 2} \\ & \vdots & \vdots & \ddots & \vdots \\ 1 & x_{1 n} & x_{2 n} & \cdots & x_{m n}\end{array}\right]((m+1) \times n-$ Matrix $)$
$\mathbf{b}:=\left[\begin{array}{c}b_{0} \\ b_{1} \\ b_{2} \\ \vdots \\ b_{m}\end{array}\right]((m+1) \times 1-$ Matrix $)$# System der Normalgleichungen 

Koeffizientenschätzung (Normalgleichungssystem in Matrixschreibweise) — wie gehabt —:
$\left(\mathbf{X}^{\mathbf{T}} \mathbf{X}\right)^{-1} \mathbf{X}_{((m+1) \times n)}^{\mathbf{T}} \cdot \mathbf{y}_{(n \times 1)}=\mathbf{b}_{((m+1) \times 1)}$
$\mathbf{b}_{((m+1) \times 1)}=\left(\mathbf{X}^{\mathbf{T}} \mathbf{X}\right)^{-1} \mathbf{X}_{((m+1) \times 2)}^{\mathbf{T}} \cdot \mathbf{y}_{(n \times 1)}$
$\mathbf{p}_{(n \times 1)}=\mathbf{X}_{((m+1) \times 2)} \cdot \mathbf{b}_{((m+1) \times 1)}$# Beispiel II Einfach-Regression bei Vorliegen eines linearen Trends 
...
| Monat $t$ | Jan | Feb | Mar | Apr | Mai | Jun | Jul | Aug | Sep | Okt | Nov | Dez |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| Beobachtungen | 30 | 25 | 37 | 43 | 39 | 55 | 66 | 59 | 48 | 53 | 49 | 57 |# Beispiel II Regression bei Trend und weiteren Einflüssen 

| Monat $t$ | Jan | Feb | Mar | Apr | Mai | Jun | Jul | Aug | Sep | Okt | Nov | Dez |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| Beobachtungen | 30 | 25 | 37 | 43 | 39 | 55 | 66 | 59 | 48 | 53 | 49 | 57 |
| Anzahl Werktage | 21 | 20 | 23 | 19 | 21 | 20 | 21 | 23 | 22 | 20 | 21 | 21 |# Beispiel II Regression bei Trend und weiteren Einflüssen 
...
Trendgerade und ex-post- und ex-ante-Prognosewerte:
$p_{t}=19.3439019+2.5024304 \cdot t+0.5304905 \cdot$ (Anzahl Werktage)# Beispiel II Regression bei Trend und weiteren Einflüssen 
...
Nachfragemenge
![img-32.jpeg](img-32.jpeg)# Beispiel II Regression bei Trend und weiteren Einflüssen 
...
Nachfragemenge
![img-33.jpeg](img-33.jpeg)![img-34.jpeg](img-34.jpeg)![img-35.jpeg](img-35.jpeg)![img-36.jpeg](img-36.jpeg)![img-37.jpeg](img-37.jpeg)# Ausgewählte Probleme bei der Einführung und Anwendung eines 
...
© Univ.-Prof. Dr. Michael Manitz# Begrenzte Vergangenheit 

Nachfragemenge
![img-38.jpeg](img-38.jpeg)# Begrenzte Vergangenheit 

Nachfragemenge
![img-39.jpeg](img-39.jpeg)# Ausreißer 

(6) Univ.-Prof. Dr. Michael Manitz# Ausreißer 
