...
**Lösungshinweise (Python-Code zur Veranschaulichung):**
...
4.  Berechnen Sie den Prognosefehler mit der mittleren absoluten Abweichung (MAE) für den 8, 9 und 10 Tag.
...
Er möchte die Methode der exponentiellen Glättung mit Trendkorrektur verwenden, wie sie im Skript vorgestellt wird. Nutzen Sie einen Glättungsfaktor $\alpha = 0.3$.

**Initialisierungswerte (zur Berechnung von $y_0^{(1)}$ und $y_0^{(2)}$):**

*   Geschätztes Niveau zum Zeitpunkt $t=0$: $\widehat{a}_0 = 480$
*   Geschätzter Trend (Steigung) zum Zeitpunkt $t=0$: $\widehat{b}_0 = 25$
...
3.  Erstellen Sie eine Prognose für die Anzahl neuer Abonnenten für Monat 9 ($p_{8+1}$) und Monat 10 ($p_{8+2}$).
...
y1_vals[0] = y0_1
y2_vals[0] = y0_2
# a_hat_vals[0] = a0_hat (per Definition)
# b_hat_vals[0] = b0_hat (per Definition)
# Alternativ: a_hat_vals[0] = 2 * y1_vals[0] - y2_vals[0]
# b_hat_vals[0] = (alpha_t3 / (1-alpha_t3)) * (y1_vals[0] - y2_vals[0]) if alpha_t3 != 1 else np.nan
# Wir verwenden die gegebenen a0_hat und b0_hat als Basis für die ersten y0^(1), y0^(2)
# und berechnen dann a_t, b_t ab t=1.
# For plotting, it's better to store a_hat_0 explicitly if we want to show it.
# However, the formulas for a_t and b_t start from t=1.
# We will plot a_t from t=1 onwards as calculated.
...
# Add original y_t to df_results_t3 for easier plotting if needed, but df_task3 already has it.
# We use df_results_t3 for calculated values.
